...
originLocationCode = 'KWJ'
destinationLocationCode = 'GMP'
# date = "2025-02-07"함
date= []
for i in range(1, 29):
    date.append(f"2025-05-{i:02d}")  # ✅ 두 자리 숫자로 포맷팅
...
def multi(i):
  ...
  try:
    conn = psycopg2.connect(host='localhost', dbname='test_db', user='postgres', password='password', port=5432)
    cursor = conn.cursor()
    try:
      for i in range(len(flight_data)):
          sql = """
          INSERT INTO test_db (departure_time, arrival_time, price, seats_available, departure_airport, arrival_airport, airline_codes)
